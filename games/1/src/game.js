...
BasicGame.GameLoad.prototype = {
	...
	preload: function() {
		this.game.load.image('preloader', GAME_PATH+'assets/preloader.png');
	},
    create: function() {
    	var p = 0;
  		this.game.stage.backgroundColor = '#24221c';
		var logo = this.game.add.sprite(0, 0, 'preloader');
		var bar = this.LoaderBar('408', '40', this.game.world.centerX-204, 600, '#1c1a16', '#53dc48');
...
BasicGame.Game.prototype = {
  	create: function(){
		...
		for(var i = 0; i < 9; i++){
			...
			if(i > 5){
				for(var n = 1; n < 11; n++){
					c.animations.add('sym_' + n, Phaser.Animation.generateFrameNames('sym_' + n + '_', 0, 19, '.png', 2), 10, false, false);
				}
			}
			this.walze1.add(c);
		}
        ...
        for(var pos = 1; pos < WLDATA.length + 1; pos++){
            ...
            var wl = game.add.bitmapData(43, 43);
            wl.ctx.beginPath();
            ...
            var label_l = this.game.add.text(startxpos + wl.width / 2, startypos + wl.height / 2 + 5, WLDATA[pos-1].line + 1, {font: "bold 24px Arial", fill: "#ffffff"});
            label_l.anchor.set(0.5);

            var label_r = this.game.add.text(startxpos2 + wl.width / 2, startypos + wl.height / 2 + 5, WLDATA[pos-1].line + 1, {font: "bold 24px Arial", fill: "#ffffff"});
        ...
        settingsbg.ctx.fillStyle = '#4c2b08';
        settingsbg.ctx.fill();
        ...
        settingsbg2.ctx.fillStyle = '#734615';
    ...
    CreateFaktorLabels: function(xpos, ypos, i){
        if(i < 10){
            var t1 = this.game.add.text(xpos + 120, ypos + 10, '3 = '+LANG[LANGUAGE]['bet']+' x' + SYM_FAKTORS[i-1]['match_3'], {font: "bold 20px Arial", fill: "#ffffff"});
            var t2 = this.game.add.text(xpos + 120, t1.y + 26, '4 = '+LANG[LANGUAGE]['bet']+' x' + SYM_FAKTORS[i-1]['match_4'], {font: "bold 20px Arial", fill: "#ffffff"});
            var t3 = this.game.add.text(xpos + 120, t2.y + 26, '5 = '+LANG[LANGUAGE]['bet']+' x' + SYM_FAKTORS[i-1]['match_5'], {font: "bold 20px Arial", fill: "#ffffff"});
            this.faktorlables.add(t1);
            this.faktorlables.add(t2);
            this.faktorlables.add(t3);
        }
        else if(i == 10){
            var t2 = this.game.add.text(xpos + 120, ypos + 36, '5 = '+LANG[LANGUAGE]['desc_jp_sym'], {font: "bold 20px Arial", fill: "#ffffff"});
            this.faktorlables.add(t2);
        }
    },
    ...
    PlayWalze1: function(){
    	...
    	this.walze1_ani = this.add.tween(this.walze1).to( { y: this.walze1.y+(200*6) }, speed, art, true);
    	if(SFX === true){
    ...
    PlayWalze2: function(){
    	...
    	this.walze2_ani = this.add.tween(this.walze2).to( { y: this.walze2.y+(200*6) }, speed, art, true, delay);
    	if(SFX === true){
    ...
    PlayWalze3: function(){
    	...
    	this.walze3_ani = this.add.tween(this.walze3).to( { y: this.walze3.y+(200*6) }, speed, art, true, delay);
    	if(SFX === true){
    ...
    PlayWalze4: function(){
    	...
    	this.walze4_ani = this.add.tween(this.walze4).to( { y: this.walze4.y+(200*6) }, speed, art, true, delay);
    	if(SFX === true){
    ...
    PlayWalze5: function(){
    	...
    	this.walze5_ani = this.add.tween(this.walze5).to( { y: this.walze5.y+(200*6) }, speed, art, true, delay);
    	if(SFX === true){
    ...
    updateCREDITS: function(){
    	$.jStorage.set(GAME_NAME+"_user_credits", CREDITS);
