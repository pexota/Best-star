...
(function (RushSlots) {
    var App = (function (_super) {
        ...
        function App() {
            ...
            this.state.add('Game_mine', RushSlots.Mine.MineGameState, false);
        ...
        var ChipsFactory = (function () {
            ...
            ChipsFactory.create = function (game, chipConfig) {
                switch (chipConfig.type) {
                    case "cave_chip":
                        return new RushSlots.Mine.ColorChip(game, chipConfig);
                }
                return null;
            };
            return ChipsFactory;
    ...
    (function (Mine) {
        var BoardView = (function (_super) {
            ...
            function BoardView(game, board) {
                ...
                this._lampOn = true;
                this._lampTimer = 0;
                this._lampNextWink = 1;
                this._lampWinkingTimer = 0;
                this._lampWinksAmount = 12;
                var sky_top = new Phaser.Image(game, 0, 0, "mine", "sky_top");
                sky_top.anchor.set(0.5, 1);
                sky_top.width = 2000;
                sky_top.height = 500;
                sky_top.y = -400;
                var skyGradient = new Phaser.Image(game, 0, 0, "mine", "sky_gradient");
                skyGradient.anchor.set(0.5, 0);
                skyGradient.width = 2000;
                skyGradient.scale.y = 1.4;
                skyGradient.y = -400;
                var sand_bg = new Phaser.Image(game, 0, 0, "mine", "sand_bg");
                sand_bg.anchor.set(0.5, 0);
                sand_bg.y = 250;
                sand_bg.width = 2000;
                sand_bg.height = 1000;
                var caveBg = new Phaser.Image(game, -285, -200, "mine", "cave_bg");
                var frame = new Phaser.Image(game, -550, -458, "mine", "cave_frame");
                //bg.position.set(45, 80);
                board.position.set(10, 30);
                var rails = new Phaser.Image(game, -630, 244, "mine", "rails");
                var cart = new Phaser.Image(game, -550, 140, "mine", "cart");
                var stones1 = new Phaser.Image(game, -104, 430, "mine", "stones_2");
                var stones2 = new Phaser.Image(game, 450, 240, "mine", "stones_2");
                stones2.scale.set(0.7, 0.7);
                var stones3 = new Phaser.Image(game, 235, 220, "mine", "stones_1");
                var touchables = [cart, stones1, stones2, stones3];
                for (var i = 0; i < touchables.length; i++) {
                    touchables[i].inputEnabled = true;
                    touchables[i].events.onInputDown.add(this.objectTouch, this, 0, touchables[i]);
                    touchables[i].anchor.set(0.5, 1);
                    touchables[i].y += touchables[i].height;
                    touchables[i].x += touchables[i].width / 2;
                }
                this._lampGalo = new Phaser.Image(game, -21, 44, "mine", "lamp_galo");
                this._lampGalo.anchor.set(0.5, 0.12);
                this._lampGalo.scale.set(5, 5);
                this._lampGalo.rotation = -Math.PI / 16;
                this._lampGalo.blendMode = Phaser.blendModes.ADD;
                this._lampGalo.alpha = 0.7;
                this._lamp = new Phaser.Image(game, -21, 44, "mine", "lamp_on");
                var lanternTop = new Phaser.Image(game, -72, 3, "mine", "lantern");
                lanternTop.inputEnabled = true;
                lanternTop.events.onInputUp.add(this.switchLamp, this);
                var lanternRing = new Phaser.Image(game, -447, -179, "mine", "light_ring");
                var lantern = new Phaser.Group(game);
                lantern.position.set(-437, -164);
                lantern.add(this._lampGalo);
                lantern.add(this._lamp);
                lantern.add(lanternTop);
                lantern.rotation = -Math.PI / 24;
                this.game.add.tween(lantern).to({ rotation: Math.PI / 10 }, 2000, Phaser.Easing.Cubic.InOut, true, 0, -1, true);
                this._ropeHook = new Phaser.Group(game);
                this._ropeHook.position.set(465, -180);
                this._ropeHook.rotation = -Math.PI / 30;
                this.game.add.tween(this._ropeHook).to({ rotation: Math.PI / 30 }, 2500, Phaser.Easing.Cubic.InOut, true, 1000, -1, true);
                var rope = new Phaser.Image(game, 0, 0, "mine", "rope");
                this._hook = new Phaser.Image(game, 18, 214, "mine", "hook");
                this._hook.pivot.set(24, 15);
                this._ropeHook.add(this._hook);
                this._ropeHook.add(rope);
                var ropeRing = new Phaser.Image(game, 435, -220, "mine", "rope_ring");
                ...
                this.add(stones2);
                this.add(stones3);
                this.add(this._ropeHook);
                this.add(ropeRing);
                this.add(lantern);
                this.add(lanternRing);
                this.pivot.set(0, -20);
            }
            BoardView.prototype.update = function () {
                ...
                if (this._lampOn) {
                    this._lampTimer += dt;
                    if (this._lampTimer > this._lampNextWink) {
                        this._lampWinkingTimer += dt;
                        if (this._lampWinkingTimer > this._lampNextWink) {
                            if (this._lampWinksAmount > 0) {
                                this.switchLight(!this._lampGalo.visible);
                                this._lampNextWink = Math.random() * 0.06;
                                this._lampWinkingTimer = 0;
                                this._lampWinksAmount--;
                            }
                            else {
                                this._lampWinksAmount = 8 + Math.floor(Math.random() * 8);
                                this._lampNextWink = 5 + Math.random() * 5;
                                this.switchLight(true);
                            }
                        }
                    }
                }
                this._hook.rotation = -this._ropeHook.rotation;
            };
            BoardView.prototype.switchLamp = function () {
                this._lampOn = !this._lampOn;
                this.switchLight(this._lampOn);
            };
            BoardView.prototype.switchLight = function (isOn) {
                this._lampGalo.visible = isOn;
                this._lamp.frameName = isOn ? "lamp_on" : "lamp_off";
            };
            BoardView.prototype.objectTouch = function (object) {
                if (!this.game.tweens.isTweening(object.scale))
                    this.game.add.tween(object.scale).to({ y: object.scale.x * 0.9, x: object.scale.y * 1.2 }, 100, Phaser.Easing.Cubic.In, true, 0, 0, true);
            };
            return BoardView;
        }(Phaser.Group));
        Mine.BoardView = BoardView;
    })(Mine = RushSlots.Mine || (RushSlots.Mine = {}));
})(RushSlots || (RushSlots = {}));
var RushSlots;
(function (RushSlots) {
    var Mine;
    (function (Mine) {
        var ColorChip = (function (_super) {
            __extends(ColorChip, _super);
            function ColorChip(game, chipConfig) {
                _super.call(this, game, chipConfig);
                var bg = new Phaser.Image(game, 0, 0, "mine", "cave_chip_" + chipConfig.color);
                bg.scale.set(0.89, 0.89);
...
(function (RushSlots) {
    var Boot = (function (_super) {
        ...
        Boot.prototype.create = function () {
            this.stage.setBackgroundColor(0x84d4cd);
...
(function (RushSlots) {
    var MenuState = (function (_super) {
        ...
        MenuState.prototype.playGame = function () {
            ...
            this.game.add.tween(this._bg).to({ alpha: 0 }, 250, null, true, 850).onComplete.addOnce(function () { _this.game.state.start('Game_mine', true, false); }, this);
        };
...
(function (RushSlots) {
    var Mine;
    (function (Mine) {
        var MineGameState = (function (_super) {
            __extends(MineGameState, _super);
            function MineGameState() {
                _super.apply(this, arguments);
            }
            MineGameState.prototype.preload = function () {
                //You can preload an image here if you dont want to use text for the loading screen
                this.load.atlasJSONHash('mine', './assets/gfx/1x/mine.png', './assets/gfx/1x/mine.json');
            };
            MineGameState.prototype.start = function () {
                _super.prototype.start.call(this);
            };
            MineGameState.prototype.create = function () {
                _super.prototype.createConfig.call(this, "mine");
                ...
                var boardView = new Mine.BoardView(this.game, this._board);
                ...
                KE.ui.ResizeManager.addSimple(boardView, 0.5, 0.46);
            };
            MineGameState.prototype.startSpin = function () {
                if (this._isSpinning)
                    return;
            ...
            return MineGameState;
        }(RushSlots.SlotsState));
        Mine.MineGameState = MineGameState;
    })(Mine = RushSlots.Mine || (RushSlots.Mine = {}));
})(RushSlots || (RushSlots = {}));
var RushSlots;
(function (RushSlots) {
